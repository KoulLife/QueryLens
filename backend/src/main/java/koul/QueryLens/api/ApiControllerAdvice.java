...
public class ApiControllerAdvice {
    ...
    public ResponseEntity<ApiResponse<?>> handleBadRequest(HttpMessageNotReadableException e) {
        ...
        } else if (rootCause instanceof DatabindException databindEx) {
            String fieldPath = databindEx.getPath().stream()
                    .map(ref -> ref.getPropertyName() != null ? ref.getPropertyName() : "?")
                    .collect(Collectors.joining("."));
            errorMessage = String.format("필드 '%s'에서 JSON 매핑 오류가 발생했습니다: %s",
                    fieldPath, databindEx.getOriginalMessage());
